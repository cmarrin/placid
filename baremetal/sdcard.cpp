...
//#define DEBUG_SD
#ifdef DEBUG_SD
#define LOG_DEBUG(args...) Serial::printf(args)
#else
#define LOG_DEBUG(args...)
#endif
#define LOG_ERROR Serial::printf

// Private functions.
static void sdParseCID(void);
static void sdParseCSD(void);
...
static int sdResetCard( int resetType )
  ...
  *EMMC_IRPT_MASK = 0xffffffff;
  ...
  LOG_DEBUG("EMMC: Status: %08x, control: %08x %08x\n",*EMMC_STATUS,*EMMC_CONTROL0,*EMMC_CONTROL1);
...
int sdInitCard()
  ...
  //  int cardAbsent = GPIO::reg(GPIO::Register::GPLEV1) & (1 << (47-32)); // TEST
  int cardAbsent = 0;
  //  int cardEjected = gpioGetEventDetected(GPIO_CD);
  int cardEjected = GPIO::reg(GPIO::Register::GPEDS1) & (1 << (47-32));
  int oldCID[4];
